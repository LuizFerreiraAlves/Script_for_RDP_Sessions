...
'@ # This type (PS_LSA) is used by Grant-UserRight, Revoke-UserRight
...
while (1)
{
    ...
    if ($Users.USERNAME.Contains('Your_user') -and ($flag_user -eq 1))
    {
        Write-Host("SUCCESS")
        $flag_user = 0
        Grant-UserRight "Usuario-PC\Your_user" SeDenyRemoteInteractiveLogonRight 
    }
    elseif (-not $Users.USERNAME.Contains('Your_user') -and ($flag_user -eq 0))
    {
        Write-Host("FAIL")
        $flag_user = 1
        Revoke-UserRight "Usuario-PC\Your_user" SeDenyRemoteInteractiveLogonRight
    }
    Write-Host($flag_user)
    Start-Sleep -s 5
}
